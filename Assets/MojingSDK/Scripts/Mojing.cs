...
public class Mojing : MonoBehaviour
{
    ...
    void Awake()
   {
    ...
    {
        string strRet = "";
        //Manufacturers ML = manufacturers_list;
        /*
        foreach (ManufacturerInfo MI in ML.ManufacturerList)
        {
            if (MI.ID != iMID)
                continue;

            Products PL = MojingSDK.GetProducts(MI.KEY, "zh");
            foreach (ProductInfo PI in PL.ProductList)
            {
                if (PI.ID != iPID)
                    continue;
                Glasses GL = MojingSDK.GetGlasses(PI.KEY, "zh");
                foreach (GlassInfo GI in GL.GlassList)
                {
                    if (GI.ID == iGID)
                    {
                        return GI.KEY;
            ...
            if ((!bWaitForMojingWord) && VRModeEnabled && NeedDistortion)
            {
                ...
                if (MojingSDK.Unity_IsATW_ON() || m_bResetTextureId)
                {
                    ...
                    if (tID == 0 || sID == 0 || tID_Metal == IntPtr.Zero || sID_Metal == IntPtr.Zero)
                    {
						...
						Debug.Log("=== texId all are null, try to get tex again");
                        m_bResetTextureId = true;
                    }
                    else
                    {
